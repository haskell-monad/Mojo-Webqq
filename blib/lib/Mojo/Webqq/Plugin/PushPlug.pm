...
sub soccer{
	...
	$client->http_get("http://www.zhibo8.cc/",{},sub{
			...
			@box = grep(/$nextDay/,@box);
			my $reply;
			if (scalar(@box) == 1) {
				$data = Mojo::DOM->new($box[0]);
				@box = $data->find('li')->each;
				@box = grep(/曼联|阿森纳|切尔西|利物浦|热刺|曼彻斯特联|曼彻斯特城|曼城|皇马|巴萨|巴塞罗那|皇家马德里|尤文|米兰|国米|拜仁|拜仁慕尼黑|多特蒙德|多特/,@box);
				my $text;
				foreach my $li (@box) {
					$dom = Mojo::DOM->new($li);
					$text = $dom->all_text;
