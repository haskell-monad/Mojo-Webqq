...
=head1 SYNOPSIS
    ...
    scala
		...
		code|scala>>>
		object Main extends App {
			val max = List(1,2,3,4,5).map((i:Int) => i * 2).filter(_%2 == 0).max
			println(max)
		}
    php
        code|php>>>
    ...
    $client->on(receive_message=>sub{
        ...
        if ($msg->content =~ m/^code\s*\|\s*([a-zA-z]+?)\s*>>>(.*)/s) {
            ...
            $client->http_post($url,{json => 1,Referer => "https://glot.io/new/$language"},json=>\%r,sub{
				...
				my $stdout = encode("utf8",$json->{stdout});
				chomp($stdout);
                if ($stdout) {
                    $client->reply_message($msg,"\@$sender_nick 执行<$language>结果如下:\n".$stdout);
                }else{
                    $client->reply_message($msg,"\@$sender_nick 执行<$language>结果如下:\n".$stdout."-------".$json->{error}."-------".$json->{stderr});
                }
            });
        }
    });
