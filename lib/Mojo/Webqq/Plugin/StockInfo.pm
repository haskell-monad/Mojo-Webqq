...
$Mojo::Webqq::Plugin::StockInfo::PRIORITY = 98;
use strict;
use 5.010;
use Encode;
...
sub call{
    ...
    $client->on(receive_message => sub {
        ...
        $client->http_get($url,sub{
			...
			$client->debug(join(@$reply_msg,"\n"));
			foreach my $reply (@$reply_msg) {
...
### $res 
	...
	push @reply,"股票名称:   $name($code)";
	push @reply,"------------------------";
	push @reply,"当前价格:$current_price\t\t" ,"开盘价格:$today_open_price";
	push @reply,"昨日收盘:$yesterday_price";
	push @reply,sprintf("幅度    :%-15s换手率  :%-17s",$UpdownPercent."%",$changePercent."%");
	push @reply,"成交量  :$totalNumber";
	push @reply,"------------------------";
	push @reply,sprintf("买一    :%-8s  %-5s卖一    :%-8s  %-5s",$buy1,$buyPrice1,$sell1,$sellPrice1);
	push @reply,sprintf("买二    :%-8s  %-5s卖二    :%-8s  %-5s",$buy2,$buyPrice2,$sell2,$sellPrice2);
	push @reply,sprintf("买三    :%-8s  %-5s卖三    :%-8s  %-5s",$buy3,$buyPrice3,$sell3,$sellPrice3);
	push @reply,sprintf("买四    :%-8s  %-5s卖四    :%-8s  %-5s",$buy4,$buyPrice4,$sell4,$sellPrice4);
	push @reply,sprintf("买五    :%-8s  %-5s卖五    :%-8s  %-5s",$buy5,$buyPrice5,$sell5,$sellPrice5);
