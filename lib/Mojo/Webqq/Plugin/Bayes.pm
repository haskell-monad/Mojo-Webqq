...
=start
	...
	ok如：记录>>>943406539,小到中雨,25℃,北风3-4级,0
		...
		预测>>>qq号码,3
	ok如：预测>>>943406539|小雨,12℃,无持续风向 ≤3级		预测用户943406539，小到中雨,25℃,北风3-4级 时是否会迟到
...
sub call{
    ...
    $client->on(receive_message=>sub{
		...
		if ($content =~ m/^(记录|save)>>>(\d+)(\||,).*,(0|1){1}$/g) {
			...
			$reply .= "\nQQ: $number\n样本集: $totals 条\n新实例:\n\tOutlook: $outlook\n\tDegree: $degree\n\tWindy: $windy\n";
			$client->reply_message($msg,"QQ: $number\t样本集: $totals 条");
			$client->reply_message($msg,"新实例:\n\tOutlook: $outlook\n\tDegree: $degree\n\tWindy: $windy");
			$reply .= "--------------开始分析--------------\n";
			#$client->reply_message($msg,$reply);
			#开始预测
			#计算先验概率
			...
			if ($final_yes && $final_no) {
				...
				$client->debug($reply);
				if ($result eq 'YES') {
					$client->reply_message($msg,"\@$sender_nick 你明天上班会迟到,小心哦.");
				}else{
					$client->reply_message($msg,"\@$sender_nick 今晚尽情折腾吧,你明天不会迟到的.");
				}
			}
